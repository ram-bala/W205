...
Step 5: Run the hive_base_ddl_extra.sql file.  This loads the measure_direction.csv file - which is a file that has an indicator for each measure that indicates whether having a high value in the metric is "better" than lower values ( by a -1) OR having a lower value means it's better (with a +1).
